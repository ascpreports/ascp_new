...
import report.pages.ypClicks;
import report.pages.ypSearchPro;
import Util.ErrorUtil;
import Util.Xls_Reader_Vivek;

public class Test_ypClicks extends TestBase{
	...
	public void initBase(){
	  ...
	  private static int rowCount_Data;
	    ...
	    public Test_ypClicks(String url, String guestMode) {
	        this.url_new = url;
	...
	public void launchURLTestypClicks(){
		HashMap<String, WebElement> hmap = new HashMap<String, WebElement>();
		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		if(!(url_new==null)){
			System.out.println(url_new);
			log.debug(url_new);
			log.debug("------- Starting Test for Launching URL ypClicks--------");
		
		//Instantiate ypSearchPro class object
		ypClicks ypClk = PageFactory.initElements(driver, ypClicks.class);
		hmap = ypClk.launchURL(url_new, guestMode_new);
		...
		log.debug("---------URL Test------- for ypClicks is Complete");
		log.debug("****************************");
		log.debug("****************************");
		log.debug("****************************");
		log.debug("****************************");
	...
	@Test(dependsOnMethods="launchURLTestypClicks", groups={"Smoke","Regression"})
	public void ypClicksOverviewTest() throws InterruptedException{
		HashMap<WebElement, String> hmap_overview = new HashMap<WebElement, String>();
		log.debug("------- Starting Test for ypClicks Overview--------");
		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		log.debug(url_new);
		//Instantiate ypSearchPro class object
		ypClicks ypClk = PageFactory.initElements(driver, ypClicks.class);
		//WebElement overview = ypAds.ypLocalAdsOverview(url);
		hmap_overview = ypClk.ypClicksOverview(guestMode_new);
		log.debug("-------# OF ELEMENTS COLLECTED IS --"+ hmap_overview.size());
		try{
			Assert.assertEquals(hmap_overview.size(), 13);
		}catch(Throwable t){
			log.debug(t);
			ErrorUtil.addVerificationFailure(t);
		}
	...
	/*@Test(dependsOnMethods="ypClicksOverviewTest", groups={"Regression"})
	public void ypClicksTopSectionTest(){
		log.debug("------- Starting Test for ypClicks Top Section--------");
		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		log.debug(url_new);
		ypClicks ypClk = PageFactory.initElements(driver, ypClicks.class);
		int i = ypClk.ypClicksTopSection();
		try{
	...
	@Test(dependsOnMethods="ypClicksOverviewTest", groups={"Regression"})
	public void ypClicksBottomSectionTest(){
		log.debug("------- Starting Test for ypClicks Bottom Section--------");
		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		log.debug(url_new);
		ypClicks ypClk = PageFactory.initElements(driver, ypClicks.class);
		int i = ypClk.ypClicksBottomSection();
		try{
	...
	public static Object[][] get_url(){
		...
		String sheetName = "ypClicks" + "_" + env;
