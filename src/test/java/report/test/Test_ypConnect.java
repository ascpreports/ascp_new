...
public class Test_ypConnect extends TestBase{
	...
	public void initBase(){
	  ...
	  private static int rowCount_Data;
	    ...
	    public Test_ypConnect(String url, String guestMode) {
	        this.url_new = url;
	...
	*/
	...
	public void launchURLTestypConnect(){
		HashMap<String, WebElement> hmap = new HashMap<String, WebElement>();
		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		if(!(url_new==null)){
			System.out.println(url_new);
			log.debug(url_new);
			log.debug("------- Starting Test for Launching URL ypConnect--------");
		
		//Instantiate ypLocalAds class object
		ypConnect ypConn = PageFactory.initElements(driver, ypConnect.class);
		hmap = ypConn.launchURL(url_new);
		...
		log.debug("---------URL Test------- for ypConnect is Complete");
		log.debug("****************************");
		log.debug("****************************");
		log.debug("****************************");
		log.debug("****************************");
	...
	@Test(dependsOnMethods="launchURLTestypConnect", groups={"Smoke","Regression"})
	public void ypConnectOverviewTest(){
		HashMap<String, WebElement> hmap_overview = new HashMap<String, WebElement>();
		log.debug("------- Starting Test for ypConnect Overview--------");
		log.debug(url_new);
		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		//Instantiate LoginPage class object
		ypConnect ypConn = PageFactory.initElements(driver, ypConnect.class);
		//WebElement overview = ypAds.ypLocalAdsOverview(url);
		hmap_overview = ypConn.ypConnectOverview(guestMode_new);
		log.debug("-------# OF ELEMENTS COLLECTED IS --"+ hmap_overview.size());
		try{
			Assert.assertEquals(hmap_overview.size(), 19);
		}catch(Throwable t){
			log.debug(t);
			ErrorUtil.addVerificationFailure(t);
		}
	...
	/*@Test(dependsOnMethods="ypWebsitesOverviewTest", groups={"Smoke","Regression"})
	...
	@Test(dependsOnMethods="ypWebsitesOverviewTest")
	...
	public static Object[][] get_url(){
		Xls_Reader_Vivek xls = new Xls_Reader_Vivek(System.getProperty("user.dir") +"\\src\\test\\java\\data\\TestCases.xlsx");
		System.out.println(xls.path);
		String sheetName = "ypConnect" + "_" + env;
		System.out.println(sheetName);
		int obj_rowcount = 0;
